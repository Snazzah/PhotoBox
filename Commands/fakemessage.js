...
module.exports = class FakeMessage extends Command {
  ...
  async exec(message, args) {
    ...
    if(!args[0]){message.channel.send("Provide text for this to work!"); return;}
    ...
    if(!text || text.trim() === ""){message.channel.send("Provide text for this to work!"); return;}
    ...
    try {
      let buffer = await this.sendToProcess(message, {
        ...
        text: text,
        channels: message.mentions.channels.array().map(c=>{return {id: c.id, name: c.name}}),
        users: message.mentions.users.array().map(u=>{return {id: u.id, name: message.mentions.members ? message.mentions.members.get(u.id).displayName : u.username}}),
        roles: message.mentions.roles.array().map(r=>{return {id: r.id, name: r.name, color: r.color, hcolor: r.hexColor}})
      })
