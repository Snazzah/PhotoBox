...
module.exports = class NekosLife extends Command {
  ...
  async exec(message, args) {
    ...
    if(this.nsfwTypes.includes(type) && !this.client.nsfw(message)) return message.reply('That tag is NSFW! Use that tag in a NSFW channel!')

    let image = await sf.get(`https://nekos.life/api/v2/img/${args[0]}`)
